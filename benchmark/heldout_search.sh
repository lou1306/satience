#!/bin/bash
# heldout_search.sh — Distributional held-out gate for SEARCH-TIME effects.
#
# Variant of heldout.sh aimed at search-time CDCL + inprocessing behavior
# (the regime where inprocessing/steering timing matters, e.g. phase-transition
# random k-SAT grind). The two key differences from heldout.sh:
#   * Harder family draws that actually stress search (larger random k-SAT,
#     larger kcolor/kclique/tseitin) rather than parity-trivialized easy draws.
#   * EXTRA_FLAGS default de-confounds the measurement: "-no-classify" disables
#     the adaptive classifier (its per-instance restartBase/cadence is overfit
#     to the fast suite and would entangle with the inprocessing cadence under
#     test), and "-parity=false" disables parity preprocessing so it cannot
#     trivialize the structured families. This isolates pure CDCL + inprocessing
#     + steering on the A/B. Override via EXTRA_FLAGS to measure a different
#     configuration (e.g. omit -no-classify for a production-config read).
#
# PURPOSE / METHOD / RAILS / ACCEPTANCE RULE are identical to heldout.sh.
#
# PURPOSE
...
#
# Usage: bash benchmark/heldout_search.sh
# Env:
...
# ---------------------------------------------------------------------------
MATRIX=(
    # === Hard random k-SAT around the phase transition (search-time CDCL
    #      grind; the regime where inprocessing timing matters). Runs with
    #      -parity=false so parity preprocessing cannot trivialize them. ===
...
run_instance() {
    ...
    s=$(date +%s.%N)
    timeout "$TIMEOUT" "$CONTROL_BINARY" $EXTRA_FLAGS "$cnf_file" > /dev/null 2>&1
    ec=$?
    e=$(date +%s.%N)
    ...
    s=$(date +%s.%N)
    timeout "$TIMEOUT" "$VARIANT_BINARY" $EXTRA_FLAGS "$cnf_file" > /dev/null 2>&1
    ec=$?
    e=$(date +%s.%N)
    d=$(awk "BEGIN{printf \"%.3f\", $e-$s}")
    case $ec in
...
# Flags passed to BOTH binaries. Default de-confounds the adaptive classifier
# (overfit to fast instances) and disables parity preprocessing, so the A/B
# isolates pure CDCL + inprocessing + steering on search-heavy draws.
EXTRA_FLAGS="${EXTRA_FLAGS:--no-classify -parity=false}"
