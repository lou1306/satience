...
- **DPLL removal** (~384 lines): Legacy `solver.go` DPLL solver, `SolveDPLL()` bridge, `-dpll` CLI flag, 7 DPLL tests. Low priority cleanup. Preserve `Assignment`/`LearnedClauseLoc` structs in `solver.go:7-25` (used by CDCL). No PAR-2 needed (not a hot-path change).
