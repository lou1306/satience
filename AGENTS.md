...
- **S5. Eliminate redundant `learnedAlive` check in propagateWatched slow path** (PAR-2 neutral): `learnedAlive` bitmap (1=alive, 0=tombstone) is fully redundant with `learnedLoc[id].Size==0` — both set in lockstep at every tombstone and cleared at compaction. Removed the two-check sequence (learnedAlive then learnedLoc) → single `learnedLoc` load + Size==0 check. Saves one array access (540ms/16.5s profile) on every slow-path watch hit. `learnedAlive` field still maintained for deletion/compaction accounting. PAR-2 1.90s best-of-3 both; per-instance deltas lean positive (-0.83s 8d58ca18, -0.73s 69d72f81, -0.39s bb34f22f; +0.45s daf59d67). Confirms C2 note: micro-opts to this hot path wash out at PAR-2 level.
...
- **Learned clauses**: Contiguous literal pool with packed `LearnedClauseLoc` (`Offset int32` + `Size int32`). Tombstone deletion (`learnedLoc[i].Size=0`); `compactLearnedClauses()` at level-0 restart when tombstones > 33%. `learnedAlive` `[]byte` bitmap maintained for deletion/compaction accounting; `propagateWatched` uses `learnedLoc[id].Size==0` (S5 — redundant check eliminated).
