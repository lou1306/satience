...
MiniSat Fast Suite (30s timeout): **72/72 solved (100%)**, PAR-2 2.44s (July 2026). Verified sound via minisat cross-check (`benchmark/cross_check_minisat.sh`): 0 mismatches.
...
- **B9. Raw PropCount deletion** (+33% PAR-2, 2 new timeouts): Cumulative counter with no recency — stale "busy" clauses protected forever. FIFO deletion (current) is better. Ban on raw PropCount; decayed activity unexplored but direction suspect.
