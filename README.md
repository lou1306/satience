...
> **🤖 AI-Built Project**: Satience was designed and implemented entirely by an AI assistant (Qwen 3.5 397B). From the core CDCL algorithm to watched literals propagation, conflict analysis, and all optimizations—every line of code was generated by AI.
...
- **Adaptive Strategy**: Analyzes instance structure before preprocessing
  - Structured instances (score ≥ 0.7): Unit propagation enabled
  - Random instances (score < 0.7): All preprocessing disabled (causes 76× more conflicts)
- **Unit Propagation** - Sound unit clause propagation before search (structured only)
- Model output (`-model`)
